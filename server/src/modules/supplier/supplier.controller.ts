...
import supplierModel from "./supplier.model.ts";

export default {
  create: asyncHandler(async (req: AdminRequest, res: Response) => {
    console.log(req.body);

    const credentials = await supplierModel
      .findOne()
      .lean({ email: req.body.email })
      .select("_id");
    if (credentials) throw new Error("Product already exist");

    const payload = await supplierModel.create(req.body);
  ...
  fetchAll: asyncHandler(async (req: Request, res: Response) => {
    ...
    const totalItems = await supplierModel.countDocuments({});
    const totalPages = Math.ceil(totalItems / 10);

    const payload = await supplierModel
      .find(filter)
      .skip(size * index)
      .limit(size)
      .lean()
  ...
  fetchById: asyncHandler(async (req: Request, res: Response) => {
    ...
    const user = await supplierModel.findById(id).lean().select("-__v");
  ...
  updateById: asyncHandler(async (req: Request, res: Response) => {
    ...
    const _updated = await supplierModel
  ...
  deleteById: asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      const credentials = await supplierModel.deleteOne({ _id: id });
